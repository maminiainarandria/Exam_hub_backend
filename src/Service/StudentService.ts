...
export class StudentService {
  ...
  async create(body: unknown) {
    if (!body || typeof body !== 'object') {
      throw new AppError(400, 'Données étudiant invalides.');
    }
  ...
  async update(id: number, body: unknown) {
    ...
    if (!existing) {
      throw new AppError(404, 'Étudiant introuvable.');
    }
    ...
    let updated = await this.users.updateStudent(
      id,
      name,
      email,
      passwordHash
    ...
    if (
      ...
      input.is_active !== updated.is_active
    ) {
      updated = await this.users.setStudentActive(
        id,
        input.is_active
      );
    }
